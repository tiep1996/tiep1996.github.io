...
export class AccountService {
  ...
  fetch(): Observable<Account> {
    return this.http.get<Account>(SERVER_API_URL + 'services/einvoiceuaa/api/account');
  }

  save(account: Account): Observable<Account> {
    return this.http.post<Account>(SERVER_API_URL + 'services/einvoiceuaa/api/account', account);
  }
  ...
  changePassword(newPassword: string, currentPassword: string): Observable<any> {
    return this.http.post(SERVER_API_URL + 'services/einvoiceuaa/api/account/change-password', {
      currentPassword,
      newPassword
    });
  }
  ...
  resetPassword(keyAndPassword: any): Observable<any> {
    return this.http.post(SERVER_API_URL + 'services/einvoiceuaa/api/account/reset-password/finish', keyAndPassword);
  }

  // forgotPassword(data): Observable<any> {
  //   return this.http.post(this.resourceUrlNotAuth + '/e-invoice/reset-password', data);
  // }

  updateUserStatus(req?: any): Observable<HttpResponse<any>> {
    const options = createRequestOption(req);
    return this.http.get<[]>(this.resourceUrl + '/e-invoice/update-user-password-status', {
      params: options,
      observe: 'response'
    });
  }
